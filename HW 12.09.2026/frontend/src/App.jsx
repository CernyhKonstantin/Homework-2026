...
import { addToCart, removeFromCart, increaseQuantity, decreaseQuantity, clearCart, selectCartItems, selectCartItemCount, selectCartTotal } from "./store/cartSlice";
...
function AddressForm({ token, onCreated }) {
  const { register, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm();
  const [message, setMessage] = useState("");
  ...
  const submit = async (values) => {
    ...
    try {
      ...
      reset();
      setMessage(`Delivery address added successfully. ID: ${data.id}`);
      onCreated();
    } catch (err) {
  ...
  return (
    <section className="card">
      ...
      <p className="muted">You can attach multiple delivery addresses to your account.</p>
      <form onSubmit={handleSubmit(submit)}>
        <label>Label</label>
        <input placeholder="Home, Work, Parents" {...register("label", { required: "Label is required", maxLength: 100 })} />
        {errors.label && <span className="error">{errors.label.message}</span>}

        <label>Recipient name</label>
        <input placeholder="John Doe" {...register("recipientName", { required: "Recipient name is required", maxLength: 200 })} />
        {errors.recipientName && <span className="error">{errors.recipientName.message}</span>}

        <div className="grid-2">
          <div>
            <label>Country</label>
            <input placeholder="Germany" {...register("country", { required: "Country is required", maxLength: 100 })} />
            {errors.country && <span className="error">{errors.country.message}</span>}
          </div>
          <div>
            <label>City</label>
            <input placeholder="Munich" {...register("city", { required: "City is required", maxLength: 100 })} />
        ...
        <div className="grid-2">
          <div>
            ...
            <input placeholder="80331" {...register("postalCode", { required: "Postal code is required", maxLength: 20 })} />
            {errors.postalCode && <span className="error">{errors.postalCode.message}</span>}
          </div>
          <div>
            <label>House number</label>
            <input placeholder="12a" {...register("houseNumber", { required: "House number is required", maxLength: 20 })} />
            {errors.houseNumber && <span className="error">{errors.houseNumber.message}</span>}
          </div>
        </div>

        <label>Street</label>
        <input placeholder="Main Street" {...register("street", { required: "Street is required", maxLength: 200 })} />
        {errors.street && <span className="error">{errors.street.message}</span>}

        <div className="grid-2">
          <div>
            <label>Apartment <span className="muted">(optional)</span></label>
            <input placeholder="Apartment 4" {...register("apartment", { maxLength: 50 })} />
          </div>
          <div>
            <label>Phone <span className="muted">(optional)</span></label>
            <input placeholder="+49 170 1234567" {...register("phone", { maxLength: 30 })} />
          </div>
        </div>

        <button disabled={isSubmitting}>{isSubmitting ? "Saving..." : "Add Delivery Address"}</button>
        {message && <div className="alert">{message}</div>}
      </form>
    </section>
  );
...
function AddressList({ token, addresses, onChanged }) {
  const [message, setMessage] = useState("");

  const removeAddress = async (id) => {
    setMessage("");
    try {
      await apiRequest(`/users/me/addresses/${id}`, { method: "DELETE" }, token);
      setMessage("Delivery address removed successfully.");
      onChanged();
    } catch (err) {
      setMessage(`Error: ${err.message}`);
    }
  ...
  return (
    <section className="card data-card">
      ...
      <p className="muted">All addresses below belong to the currently authenticated user.</p>
      {message && <div className="alert">{message}</div>}
      {addresses.length === 0 ? (
        <p className="muted">No delivery addresses yet.</p>
      ) : (
          ...
          {addresses.map(address => (
            <article className="address-item" key={address.id}>
              <div>
                ...
                <div>{address.street} {address.houseNumber}{address.apartment ? `, ${address.apartment}` : ""}</div>
                <div>{address.postalCode} {address.city}, {address.country}</div>
                {address.phone && <div>{address.phone}</div>}
              </div>
              <button className="danger" onClick={() => removeAddress(address.id)}>Remove</button>
            </article>
          ))}
        </div>
      )}
...
function ProductCatalog({ products }) {
  ...
  return (
    <section className="card data-card">
      ...
      <p className="muted">Add products to the Redux-managed shopping cart.</p>
      {products.length === 0 ? (
          ...
          {products.map(product => (
            <article className="product-card" key={product.id}>
              <div>
                ...
                <span className="product-stock">Stock: {product.stockQty}</span>
              </div>
              <button
                disabled={!product.isActive || product.stockQty <= 0}
                onClick={() => dispatch(addToCart(product))}
              >
                {!product.isActive ? "Inactive" : product.stockQty <= 0 ? "Out of stock" : "Add to cart"}
              </button>
            </article>
          ))}
        </div>
  ...
  return (
    <section className="card cart-card">
      <div className="cart-header">
        <div>
          <h2>Shopping Cart</h2>
          <p className="muted">Redux store manages all cart state and actions.</p>
        </div>
        <span className="cart-count">{itemCount} item{itemCount === 1 ? "" : "s"}</span>
      </div>

      {items.length === 0 ? (
        <p className="muted">Your cart is empty. Add a product from the catalog.</p>
      ) : (
        <>
          <div className="cart-list">
            {items.map(item => (
              <article className="cart-item" key={item.productId}>
                ...
                <button className="danger small-button" onClick={() => dispatch(removeFromCart(item.productId))}>Remove</button>
              </article>
            ))}
          </div>
...
function CategoryLookup() {
  ...
  const submit = async (values) => {
    ...
    setError("");
    try {
      const path = values.mode === "id"
        ? `/categories/${Number(values.value)}`
        : `/categories/${encodeURIComponent(values.value.trim())}`;
  ...
  return (
    <section className="card">
      <h2>Category Queries</h2>
      <p className="muted">These requests are handled by MediatR query handlers.</p>
      <form onSubmit={handleSubmit(submit)}>
        <label>Query type</label>
        <select {...register("mode")}>
          <option value="id">Get category by ID</option>
          <option value="slug">Get category by slug</option>
        </select>
        <label>ID or slug</label>
        <input placeholder="1 or electronics" {...register("value", { required: "Value is required" })} />
        <button disabled={isSubmitting}>{isSubmitting ? "Loading..." : "Run Query"}</button>
      </form>
      {error && <div className="alert error-box">{error}</div>}
...
export default function App() {
  ...
  const loadAddresses = async () => {
    ...
    try {
      const data = await apiRequest("/users/me/addresses", {}, auth.accessToken);
      setAddresses(data || []);
    } catch (err) {
  ...
  useEffect(() => { loadData(); }, []);
  useEffect(() => { if (auth) loadAddresses(); else setAddresses([]); }, [auth]);
  ...
  if (!auth) {
    return <main className="page"><header><h1>HW 12.09.2026</h1><p>Category & Product Creation</p></header><LoginForm onLogin={setAuth} /><div className="hint">Default seeded admin: <strong>admin@example.com</strong> / <strong>ChangeMe123!</strong></div></main>;
  }

  return (
    <main className="page">
      <header className="topbar"><div><h1>HW 12.09.2026</h1><p>React Hook Form + ASP.NET Core + SQL Server</p></div><div className="user"><span>{auth.email} · {auth.role}</span><button className="secondary" onClick={logout}>Logout</button></div></header>
      ...
      <div className="forms">
        ...
        <CategoryLookup />
        <AddressForm token={auth.accessToken} onCreated={loadAddresses} />
      </div>
      {loadError && <div className="alert error-box">Could not load data: {loadError}</div>}
      <AddressList token={auth.accessToken} addresses={addresses} onChanged={loadAddresses} />
