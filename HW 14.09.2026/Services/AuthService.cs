...
using HW_14._09._2026.DTOs.Auth;
using HW_14._09._2026.Helpers;
using HW_14._09._2026.Models;
using HW_14._09._2026.Repositories.Interfaces;
using HW_14._09._2026.Services.Interfaces;

namespace HW_14._09._2026.Services;
...
public class AuthService : IAuthService
{
    ...
    private readonly IUserQueuePublisher _userQueuePublisher;
    ...
    public AuthService(
        ...
        IUserQueuePublisher userQueuePublisher,
        IProviderRepository providerRepository,
        IUserProviderRepository userProviderRepository)
    {
        _userQueuePublisher = userQueuePublisher;
    ...
    public async Task<RegisterResponseDto?> RegisterAsync(RegisterDto dto, HttpResponse response)
    {
        ...
        await _userQueuePublisher.PublishUserRegisteredAsync(user.Id, user.Email);
    ...
    public async Task<LoginResponseDto?> ExternalResponseAsync(
        ExternalAuthDto dto,
        HttpResponse response,
        CancellationToken cancellationToken = default)
    {
        var email = dto.Email.Trim().ToLowerInvariant();
        var providerName = dto.Provider.Trim().ToLowerInvariant();

        if (providerName == "facebook")
        {
            providerName = "fb";
        }
        var numberProvider = dto.NumberProvider.Trim();

        if (string.IsNullOrWhiteSpace(email) ||
            string.IsNullOrWhiteSpace(providerName) ||
            string.IsNullOrWhiteSpace(numberProvider))
        {
            return null;
        }

        var provider = await _providerRepository.GetByNameAsync(providerName, cancellationToken);
        ...
        var user = await _userRepository.GetByEmailAsync(email);
        var existingProviderOwner = await _userProviderRepository.GetByProviderNumberAsync(
            provider.Id, numberProvider, cancellationToken);

        if (existingProviderOwner != null &&
            (user == null || existingProviderOwner.UserId != user.Id))
        {
            return null;
        }
        ...
        var existingLink = await _userProviderRepository.GetAsync(
            user.Id, provider.Id, cancellationToken);

        if (existingLink == null)
        {
            await _userProviderRepository.CreateAsync(new UserProvider
            {
                UserId = user.Id,
        ...
        else if (!string.Equals(existingLink.NumberProvider, numberProvider, StringComparison.Ordinal))
        {
            return null;
        }
