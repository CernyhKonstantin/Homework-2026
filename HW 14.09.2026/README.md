...
## HW 14.09.2026 - React Hook Form Category and Product Creation
...
## HW 14.09.2026 - Multiple User Delivery Addresses
...
## Redux Shopping Cart (HW 14.09.2026)

The React frontend now uses Redux Toolkit to centralize shopping cart management.
...
The Redux store is provided to React through `Provider` in `frontend/src/main.jsx`. Products can be added from the product catalog and quantities can be increased, decreased, removed, or cleared from the cart.

## External authentication providers

This homework adds external-account linking for **Google**, **Facebook (`fb`)**, and **Apple**.

### Database models

- `Provider`: `Id`, `Name`
- `UserProvider`: `Id`, `UserId`, `ProviderId`, `NumberProvider`, `CreatedAt`
- Providers are seeded with:
  - `google`
  - `fb`
  - `apple`
...
```text
Migrations/20260912000000_AddExternalProviders.cs
```

The database uses the requested `user_id`, `provider_id`, `number_provider`, and `created_at` column names for `UserProvider`.

### ExternalResponse endpoint
...
```json
{
  ...
  "provider": "google",
  "numberProvider": "1234567890"
}
```

The endpoint performs the following flow:

1. Validates the external provider.
2. Looks up the user by email.
3. Creates the user when the email does not exist yet.
4. Links the external provider account through `UserProvider`.
5. Prevents the same provider account from being linked to another user.
6. Issues a JWT access token.
7. Creates and returns a refresh token and stores it in the secure HTTP-only `refresh_token` cookie.

The controller also accepts and forwards a `CancellationToken` to the database operations involved in external authentication.

## Homework 12.09.2026 summary

This version contains the complete previous homework functionality plus external authentication account persistence.

### External authentication flow

`POST /api/Auth/external` accepts an email, provider (`google`, `fb`, or `apple`), and provider account number. If the email is not present in `Users`, a new local user is created. The external account is stored in `UserProviders`. The endpoint then issues both access and refresh JWT tokens using the existing authentication infrastructure.

The refresh token is returned in the response DTO and also stored in the HTTP-only `refresh_token` cookie. A `CancellationToken` is accepted by `AuthController.ExternalResponse()` and propagated to provider/user-provider persistence operations.


## HW 14.09.2026 - ASP.NET Core MVC

This homework adds an MVC interface on top of the existing Web API. The MVC layer reuses services from the previous project instead of accessing Entity Framework directly.

### MVC Controllers

- `CategoriesController` - displays all categories using `ICategoryService`.
- `UsersController` - displays all users using `IAdminService`.

### MVC Menu

The MVC layout contains two navigation items:
...
Sensitive user information such as password hashes is never rendered by the MVC user view.
