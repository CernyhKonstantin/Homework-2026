...
```powershell
docker exec -it hw14082026-redis redis-cli ping
```
...
The application now includes a browser frontend in `wwwroot`.

Features:

- Registration with email, password, and repeated password
- Login with email and password
- Access token stored in browser localStorage
- Refresh token stored by the client and also issued as an HttpOnly cookie
- Logout
- Header greeting: `Welcome, email`
- Access token refresh

Start the API and open the root URL. The authentication page is served by ASP.NET Core.
